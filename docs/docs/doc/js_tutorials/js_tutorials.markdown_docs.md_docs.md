# Documentation for `docs/doc/js_tutorials/js_tutorials.markdown_docs.md`

## File Metadata

- **Full Path**: `docs/doc/js_tutorials/js_tutorials.markdown_docs.md`
- **File Name**: `js_tutorials.markdown_docs.md`
- **File Size**: 1,620 bytes
- **File Type**: .md
- **Link to Source**: [docs/doc/js_tutorials/js_tutorials.markdown_docs.md](../../../docs/doc/js_tutorials/js_tutorials.markdown_docs.md)

## Purpose and Role

This file is located in the `docs/doc/js_tutorials` directory and serves as part of the OpenCV library infrastructure.

## Documentation Content

# Documentation for `doc/js_tutorials/js_tutorials.markdown`

## File Metadata

- **Full Path**: `doc/js_tutorials/js_tutorials.markdown`
- **File Name**: `js_tutorials.markdown`
- **File Size**: 1,028 bytes
- **File Type**: .markdown
- **Link to Source**: [doc/js_tutorials/js_tutorials.markdown](../../doc/js_tutorials/js_tutorials.markdown)

## Purpose and Role

This file is located in the `doc/js_tutorials` directory and serves as part of the OpenCV library infrastructure.

## File Content

```
OpenCV.js Tutorials {#tutorial_js_root}
=======================
-   @subpage tutorial_js_table_of_contents_setup

    Learn how to use OpenCV.js inside your web pages!

-   @subpage tutorial_js_table_of_contents_gui

    Here you will learn how to read and display images and videos, and create trackbar.

-   @subpage tutorial_js_table_of_contents_core

    In this section you will learn some basic operations on image, some mathematical tools and some data structures etc.

-   @subpage tutorial_js_table_of_contents_imgproc

    In this section
    you will learn different image processing functions inside OpenCV.

-   @subpage tutorial_js_table_of_contents_video

    In this section you
    will learn different techniques to work with videos like object tracking etc.

-   @subpage tutorial_js_table_of_contents_objdetect

    In this section you
    will object detection techniques like face detection etc.

-   @subpage tutorial_js_table_of_contents_dnn

    These tutorials show how to use dnn module in JavaScript

```

## General Information

This file is part of the OpenCV repository infrastructure.



## Documentation Purpose

This file provides documentation, guides, or README information for users and developers of OpenCV.

