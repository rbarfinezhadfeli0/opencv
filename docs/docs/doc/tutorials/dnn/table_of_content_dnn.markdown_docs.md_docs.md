# Documentation for `docs/doc/tutorials/dnn/table_of_content_dnn.markdown_docs.md`

## File Metadata

- **Full Path**: `docs/doc/tutorials/dnn/table_of_content_dnn.markdown_docs.md`
- **File Name**: `table_of_content_dnn.markdown_docs.md`
- **File Size**: 1,762 bytes
- **File Type**: .md
- **Link to Source**: [docs/doc/tutorials/dnn/table_of_content_dnn.markdown_docs.md](../../../../docs/doc/tutorials/dnn/table_of_content_dnn.markdown_docs.md)

## Purpose and Role

This file is located in the `docs/doc/tutorials/dnn` directory and serves as part of the OpenCV library infrastructure.

## Documentation Content

# Documentation for `doc/tutorials/dnn/table_of_content_dnn.markdown`

## File Metadata

- **Full Path**: `doc/tutorials/dnn/table_of_content_dnn.markdown`
- **File Name**: `table_of_content_dnn.markdown`
- **File Size**: 1,122 bytes
- **File Type**: .markdown
- **Link to Source**: [doc/tutorials/dnn/table_of_content_dnn.markdown](../../../doc/tutorials/dnn/table_of_content_dnn.markdown)

## Purpose and Role

This file is located in the `doc/tutorials/dnn` directory and serves as part of the OpenCV library infrastructure.

## File Content

```
Deep Neural Networks (dnn module) {#tutorial_table_of_content_dnn}
=====================================

@tableofcontents

-   @subpage tutorial_dnn_googlenet
-   @subpage tutorial_dnn_halide
-   @subpage tutorial_dnn_halide_scheduling
-   @subpage tutorial_dnn_openvino
-   @subpage tutorial_dnn_yolo
-   @subpage tutorial_dnn_javascript
-   @subpage tutorial_dnn_custom_layers
-   @subpage tutorial_dnn_OCR
-   @subpage tutorial_dnn_text_spotting
-   @subpage tutorial_dnn_face

#### PyTorch models with OpenCV
In this section you will find the guides, which describe how to run classification, segmentation and detection PyTorch DNN models with OpenCV.
-   @subpage pytorch_cls_tutorial_dnn_conversion
-   @subpage pytorch_cls_c_tutorial_dnn_conversion
-   @subpage pytorch_segm_tutorial_dnn_conversion

#### TensorFlow models with OpenCV
In this section you will find the guides, which describe how to run classification, segmentation and detection TensorFlow DNN models with OpenCV.
-   @subpage tf_cls_tutorial_dnn_conversion
-   @subpage tf_det_tutorial_dnn_conversion
-   @subpage tf_segm_tutorial_dnn_conversion

```

## General Information

This file is part of the OpenCV repository infrastructure.



## Documentation Purpose

This file provides documentation, guides, or README information for users and developers of OpenCV.

