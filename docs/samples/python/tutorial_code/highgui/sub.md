# Subtree Keyword Index for `samples/python/tutorial_code/highgui/`

## Scope

This keyword index covers **all files** under `samples/python/tutorial_code/highgui/` recursively, including all subdirectories.

**Statistics:**
- **Total Files in Subtree**: 1
- **Total Unique Keywords**: 86
- **Direct Files**: 0
- **Subfolders**: 1

## Keywords A-Z

The following keywords, identifiers, and technical terms appear in files within this subtree:


### A

**Adding** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**AddingImagesTrackbar** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**Alpha** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**ArgumentParser** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**addWeighted** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**add_argument** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**alpha** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**alpha_slider_max** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**and** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**applications** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**argparse** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**args** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### B

**Blend** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**beta** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**both** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### C

**Code** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**Could** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**createTrackbar** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**create_trackbar** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**cv2** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### D

**def** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**default** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**description** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**division** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**dst** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### E

**exit** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### F

**find** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**findFile** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**first** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**for** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**from** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### H

**have** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**help** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**highgui** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### I

**image** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**images** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**import** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**imread** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**imshow** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**input** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**input1** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**input2** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### J

**jpg** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### K

**key** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### L

**Linear** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**LinuxLogo** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**load** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### N

**None** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**namedWindow** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**not** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### O

**on_trackbar** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**open** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**our** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### P

**Path** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**parse_args** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**parser** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**press** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**print** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**print_function** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**python** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### R

**Read** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### S

**Show** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**same** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**samples** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**second** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**size** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**some** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**src1** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**src2** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**stuff** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### T

**Trackbar** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**the** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**title_window** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**trackbar** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**trackbar_name** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**tutorial** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**tutorial_code** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**type** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### U

**until** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**user** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### V

**val** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### W

**Wait** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**WindowsLogo** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**waitKey** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))

**window** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))


### _

**__future__** - Found in 1 file:
  - [samples/python/tutorial_code/highgui/trackbar/AddingImagesTrackbar.py](trackbar/AddingImagesTrackbar.py_docs.md) ([keywords](trackbar/AddingImagesTrackbar.py_kw.md))



## Folder-Level Navigation

### Keyword Distribution by Subfolder

- **[trackbar/](trackbar/sub.md)**: 1 files


### Most Common Keywords

The most frequently appearing keywords in this subtree:

- **python**: 1 occurrences
- **images**: 1 occurrences
- **from**: 1 occurrences
- **press**: 1 occurrences
- **jpg**: 1 occurrences
- **imshow**: 1 occurrences
- **size**: 1 occurrences
- **src1**: 1 occurrences
- **load**: 1 occurrences
- **parser**: 1 occurrences
- **stuff**: 1 occurrences
- **__future__**: 1 occurrences
- **Trackbar**: 1 occurrences
- **import**: 1 occurrences
- **samples**: 1 occurrences
- **first**: 1 occurrences
- **val**: 1 occurrences
- **have**: 1 occurrences
- **window**: 1 occurrences
- **open**: 1 occurrences
- **not**: 1 occurrences
- **tutorial_code**: 1 occurrences
- **input2**: 1 occurrences
- **find**: 1 occurrences
- **same**: 1 occurrences
- **WindowsLogo**: 1 occurrences
- **on_trackbar**: 1 occurrences
- **Code**: 1 occurrences
- **cv2**: 1 occurrences
- **tutorial**: 1 occurrences
- **createTrackbar**: 1 occurrences
- **user**: 1 occurrences
- **AddingImagesTrackbar**: 1 occurrences
- **help**: 1 occurrences
- **waitKey**: 1 occurrences
- **alpha_slider_max**: 1 occurrences
- **namedWindow**: 1 occurrences
- **dst**: 1 occurrences
- **image**: 1 occurrences
- **Show**: 1 occurrences
- **Alpha**: 1 occurrences
- **alpha**: 1 occurrences
- **type**: 1 occurrences
- **print_function**: 1 occurrences
- **print**: 1 occurrences
- **description**: 1 occurrences
- **Linear**: 1 occurrences
- **add_argument**: 1 occurrences
- **both**: 1 occurrences
- **Wait**: 1 occurrences
- **def**: 1 occurrences
- **imread**: 1 occurrences
- **create_trackbar**: 1 occurrences
- **some**: 1 occurrences
- **second**: 1 occurrences
- **input1**: 1 occurrences
- **for**: 1 occurrences
- **exit**: 1 occurrences
- **trackbar_name**: 1 occurrences
- **division**: 1 occurrences
- **our**: 1 occurrences
- **highgui**: 1 occurrences
- **default**: 1 occurrences
- **findFile**: 1 occurrences
- **Path**: 1 occurrences
- **title_window**: 1 occurrences
- **key**: 1 occurrences
- **Adding**: 1 occurrences
- **src2**: 1 occurrences
- **until**: 1 occurrences
- **ArgumentParser**: 1 occurrences
- **Read**: 1 occurrences
- **addWeighted**: 1 occurrences
- **the**: 1 occurrences
- **input**: 1 occurrences
- **parse_args**: 1 occurrences
- **argparse**: 1 occurrences
- **trackbar**: 1 occurrences
- **Could**: 1 occurrences
- **applications**: 1 occurrences
- **Blend**: 1 occurrences
- **LinuxLogo**: 1 occurrences
- **args**: 1 occurrences
- **beta**: 1 occurrences
- **None**: 1 occurrences
- **and**: 1 occurrences
